...
router.get('/', async (req, res, next) => {
	const {id, name, country, specializationId, service, filter} = req.query;
	try{
		res.result = await hospital.getHospital({id, name, country, specializationId, service, filter}, req.paginator);
		next();
	}catch(err){
		next(err);
	}
